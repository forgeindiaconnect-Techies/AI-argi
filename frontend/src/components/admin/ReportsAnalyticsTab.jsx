...
const ReportsAnalyticsTab = () => {
  ...
  const openPdfWindow = (title, headings, rows) => {
      ...
      <html>
        <head>
          ...
          <style>
            * { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .logo { width: 40px; height: 40px; background: #16a34a; border-radius: 8px; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 18px; }
            h1 { color: #16a34a; font-size: 22px; }
            h2 { color: #374151; font-size: 16px; margin-bottom: 16px; }
        ...
        <body>
          <div class="header">
            ...
            <div>
              <h1>SAMS Admin Portal</h1>
              <div class="meta">Smart Agriculture Management System &bull; ${title}</div>
            </div>
          </div>
          <h2>${title}</h2>
  ...
  const handleDownload = (format) => {
    if (format === 'pdf') {
      if (activeTab === 'Crop Performance') {
        openPdfWindow('Crop Performance Report',
          ['Crop', 'Target Yield (Tons)', 'Actual Yield (Tons)'],
          cropPerformanceData.map(r => [r.name, r.target, r.actual])
        );
      } else if (activeTab === 'Yield Reports') {
        openPdfWindow('Yield Reports',
          ['Report ID', 'Crop', 'Season', 'Total Area', 'Total Yield', 'Variance'],
          yieldReportData.map(r => [r.id, r.crop, r.season, r.area, r.totalYield, r.variance])
        );
      } else if (activeTab === 'Farmer Reports') {
        openPdfWindow('Farmer Reports',
          ['Farmer ID', 'Name', 'Region', 'Yield', 'Status'],
          farmerReportData.map(r => [r.id, r.name, r.region, r.yield, r.status])
        );
      } else if (activeTab === 'Soil Reports') {
        openPdfWindow('Soil Analysis Reports',
          ['Test ID', 'Region / Farm', 'pH', 'Nitrogen', 'Phosphorus', 'Moisture'],
          soilReportData.map(r => [r.testId, r.region, r.ph, r.nitrogen, r.phosphorus, r.moisture])
        );
      } else if (activeTab === 'Weather Reports') {
        openPdfWindow('Weather Reports',
          ['Period', 'Avg Temp', 'Rainfall', 'Humidity'],
          weatherReportData.map(r => [r.month, r.avgTemp, r.rainfall, r.humidity])
        );
      } else if (activeTab === 'AI Reports') {
        openPdfWindow('AI Analysis Reports',
          ['Date', 'Farmer', 'Report Type', 'Crop', 'Risk Level'],
          aiReportData.map(r => [r.date, r.farmerName, r.type, r.crop, r.risk])
        );
      }
      return;
    }
    ...
    if (activeTab === 'Crop Performance') {
      ...
      reportContent += 'Period,Avg Temp,Rainfall,Humidity\n';
      weatherReportData.forEach(r => reportContent += `${r.month},${r.avgTemp},${r.rainfall},${r.humidity}\n`);
    } else if (activeTab === 'AI Reports') {
      reportContent += 'Date,Farmer,Type,Crop,Risk\n';
      aiReportData.forEach(r => reportContent += `${r.date},${r.farmerName},${r.type},${r.crop},${r.risk}\n`);
    }
                    ...
                    <td className="p-4 text-right">
                      <button
                        onClick={() => openPdfWindow(
                          `AI Report — ${row.type}`,
                          ['Field', 'Value'],
                          [['Date', row.date], ['Farmer', row.farmerName], ['Report Type', row.type], ['Crop', row.crop], ['Risk Level', row.risk]]
                        )}
                        className="text-agri-green hover:text-agri-green-dark inline-flex items-center gap-1"
                      >
                        <Download className="w-4 h-4"/> PDF
                      </button>
                    </td>
